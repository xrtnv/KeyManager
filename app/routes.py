...
async def generate_secret(secret_request: SecretRequest):
    ...
    await db.secrets.insert_one(secret.model_dump(by_alias=True))
...
async def get_secret(secret_key: str, passphrase: str = Query(...)):
    ...
    if (not secret or not verify_password(passphrase, secret["hashed_passphrase"]) or
            secret['expires_at'] < datetime.now()):
